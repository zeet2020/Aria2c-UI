...
export default class Api {
  ...
  savePreferenceToNativeStore (params = {}) {
    ...
    if (!isEmpty(user)) {
      console.info('[AUI] save user config: ', user)
      config.user = user
    }
    ...
    if (!isEmpty(others)) {
      console.info('[AUI] save config found illegal key: ', others)
    }
  ...
  fetchDownloadingTaskList (params = {}) {
    ...
    return new Promise((resolve, reject) => {
      this.client.multicall([
        ...
        console.log('[AUI] fetch downloading task list data:', data)
        const result = mergeTaskResult(data)
        resolve(result)
      }).catch((err) => {
        console.log('[AUI] fetch downloading task list fail:', err)
        reject(err)
      })
    })
  }
  ...
  fetchTaskItemWithPeers (params = {}) {
    ...
    return new Promise((resolve, reject) => {
      this.client.multicall([
        ...
        console.log('[AUI] fetchTaskItemWithPeers:', data)
        const result = data[0] && data[0][0]
        const peers = data[1] && data[1][0]
        result.peers = peers || []
        console.log('[AUI] fetchTaskItemWithPeers.result:', result)
        console.log('[AUI] fetchTaskItemWithPeers.peers:', peers)
