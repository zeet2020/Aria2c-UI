...
export default class CommandManager extends EventEmitter {
  ...
  register (id, fn) {
    if (this.commands[id]) {
      console.log('[AUI] Attempting to register an already-registered command: ' + id)
      return null
    }
    if (!id || !fn) {
      console.error('[AUI] Attempting to register a command with a missing id, or command function.')
      return null
    }
    this.commands[id] = fn
