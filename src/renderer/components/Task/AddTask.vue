        ...
        <el-tab-pane :label="$t('task.uri-task')" name="uri">
          <el-form-item>
            <el-input
              ...
              @dragover.prevent
              @drop.prevent="handleUriDrop"
              v-model="form.uris"
            >
            </el-input>
            <div class="uri-upload">
              <span class="action-link" @click="triggerTxtUpload">
                <mo-icon name="inbox" width="12" height="12" />
                {{ $t('task.upload-txt') }}
              </span>
              <input
                ref="txtInput"
  ...
  export default {
    ...
    methods: {
      async autofillResourceLink () {
        // Clipboard read can be unavailable/blocked in the webview; autofill is
      ...
      async loadUrisFromTxt (file) {
        const name = (file.name || '').toLowerCase()
        if (!name.endsWith('.txt') && file.type !== 'text/plain') {
          this.$msg.error(this.$t('task.upload-txt-invalid'))
          return
        }
        try {
          const text = await file.text()
          this.form.uris = text.trim() // replace existing content
          this.detectThunderResource(this.form.uris)
        } catch (err) {
      ...
      addTask (type, form) {
        ...
        // @TODO addMetalink
          ...
          console.error('[AUI] Add task fail', form)
        }
      },
      submitForm (formName) {
